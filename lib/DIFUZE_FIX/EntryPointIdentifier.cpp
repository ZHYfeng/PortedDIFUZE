#ifndef PROJECT_ENTRY_H_FIX
#define PROJECT_ENTRY_H_FIX
...
namespace IOCTL_CHECKER_FIX {
    ...
    INT_STS kernel_sts[]{
            ...
            {"struct.bin_attribute",                   4,  READ_HDR},
            {"struct.bin_attribute",                   5,  WRITE_HDR},
            {"struct.atmdev_ops",                      3,  IOCTL_HDR},
            {"struct.atmphy_ops",                      1,  IOCTL_HDR},
            {"struct.atm_ioctl",                       1,  IOCTL_HDR},
            {"struct.kernfs_ops",                      6,  READ_HDR},
            {"struct.kernfs_ops",                      9,  WRITE_HDR},
            ...
            {"struct.rtc_class_ops",                   0,  IOCTL_HDR},
            {"struct.net_device_ops",                  11, IOCTL_HDR},
            {"struct.kvm_device_ops",                  8,  IOCTL_HDR},
            {"struct.ide_disk_ops",                    8,  IOCTL_HDR},
            {"struct.ide_ioctl_devset",                0,  IOCTL_HDR},
            {"struct.ide_ioctl_devset",                1,  IOCTL_HDR},
            {"struct.pci_ops",                         3,  READ_HDR},
            {"struct.pci_ops",                         4,  WRITE_HDR},
            {"struct.cdrom_device_info",               10, IOCTL_HDR},
            {"struct.cdrom_device_ops",                11, IOCTL_HDR},
            ...
            {"struct.uart_ops",                        22, IOCTL_HDR},
            {"struct.tty_ldisc_ops",                   7,  READ_HDR},
            {"struct.tty_ldisc_ops",                   8,  WRITE_HDR},
            {"struct.tty_ldisc_ops",                   9,  IOCTL_HDR},
            {"struct.fb_ops",                          16, IOCTL_HDR},
            {"struct.v4l2_subdev_core_ops",            6,  IOCTL_HDR},
            ...
            {"struct.snd_hwdep_ops",                   7,  IOCTL_COMPAT_HDR},
            ...
            {"struct.posix_clock_operations",          5,  IOCTL_HDR},
            {"struct.posix_clock_operations",          9,  READ_HDR},
            {"struct.block_device_operations",         4,  IOCTL_HDR},
            ...
            {"struct.v4l2_file_operations",            5,  IOCTL_COMPAT_HDR},
                    ...
                    if (kernel_sts[i].st_name == containedType->getStructName()) {
                        if (currGlobal->hasInitializer()) {
                            ...
                            if (targetFunction != nullptr && !targetFunction->isDeclaration() &&
                                ...
                                yhao_log(0, "*****************************************");
                                add_entry_point(currGlobal->getName().str(), targetFunction,
                                                std::string(kernel_sts[i].method_lab));
                            } else {
                                yhao_log(0, "*****************************************");
                                yhao_log(0,
